...
%Résumé
\section{Résumé}
Le projet consiste à gérer un cube à led 3D composé de 512 led avec une nouvelle application plus adapté que les versions précédentes. Comparé aux versions précédentes, notre version apportera une vision 3D du cube à led sur l'ordinateur \\

Pour créer cette application, il nous aura fallut recommencer depuis le début, créer un nouveau projet avec notre propre dll pour communiquer avec le cube à led. \\

Le langage de programmation C\# est utilisé pour l'application. Pour la conception de la virtualisation du cube à led en 3D, nous avons utilisé monogame pour la création d'objets en 3D. \\

En conclusion, la communication et la virtualisation du cube à led en 3D fonctionne, mais quelque fonction comme le picking ne sont pas disponible. Une grosse partie du projet a été dédié au picking

\subsection{Abstract}
The project consists to managing a 3D led cube made up of 512 led with a new application more adapted than the previous versions. Compared to previous versions, our version will bring a 3D vision of the LED cube on the computer\\

To create this application, we had to start over from the beginning, create a new project with our own dll to communicate with the led cube.\\

The C\# programming language is used for the application. For designing 3D cube to 3D virtualization, we used monogame to create 3D objects.\\

In conclusion, the communication and virtualization of the cube to 3D led works, but some function like picking are not available. A large part of the project was devoted to the picking.
\newpage
...
L'interface (\addRef{interface}) contient les éléments essentiels pour "jouer" avec le cube. Au milieu, dans la partie verte claire, on retrouve le cube à LED matérialisé en 3D. L'utilisateur pourra sélectionner les leds du cube et les modifier grâce aux interactions que l'on trouve en dessous.
\vspace{.5cm}
